...
### Database search and 3D window (independent of the GPU gateway)

Atlas UniParc IDs can also resolve through the official UniParc API: both archive and UniProt sequences must exactly match. Only an active UniProt cross-reference is considered, preferring reviewed records; reference species are explicitly not asserted to be the user's specimen identity. No similarity inference is used. The complete 238-aa GFP sequence was also tested through this raw-sequence route.

Stage 2 also accepts a protein/gene name or UniProt accession, with an optional organism filter. UniProt returns up to eight identity choices; the user confirms the species/accession before loading a structure. Selected accessions resolve to the complete current UniProt sequence, then query AlphaFold DB. The adapter supports the new AFDB `sequence`, `sequenceStart`, `sequenceEnd` and `modelEntityId` fields, retaining old-field compatibility. If the prediction metadata API is unavailable, official EMBL-EBI 3D-Beacons metadata supplies the exact versioned AFDB model location; its companion PDB is independently checked against the complete UniProt sequence. Fixed provider hosts, bounded responses and no redirects prevent arbitrary URL fetches. Biohub Atlas is a second exact-match source.

Raw sequence/FASTA inputs query Biohub Atlas by content hash. If Atlas supplies an explicit UniProt accession, that sequence is rechecked before looking up AlphaFold. Otherwise a missing Atlas structure remains a miss; users can try the name/accession path. Similar proteins and partial models are not silently substituted. All downloaded database models are labeled **existing predictions**, never new AlphaFold 3 jobs or experimental structures. No inference API key or paid credits are used. Database lookup accepts 1–5000 canonical residues with bounded PDB size; this is an application safety limit, not unlimited model capacity. Local new prediction remains 10–200 aa. Database-only results do not imply that topology, SASA or functional-site analysis was run.

The shared 3D viewer now includes a jump button and a full-screen control. On 2026-09-24, a real UniProt P42212 GFP query retrieved the 238-aa AlphaFold v6 PDB through the official 3D-Beacons fallback; all 238 residue identities matched, and CA-derived mean pLDDT was 96.64/100. The direct AFDB metadata endpoint returned HTTP 403 from the local environment, and the report retains that failure alongside the successful fallback. Biohub had a record but no coordinates for this example. This is a real database retrieval, not fresh inference or experimental validation.

Read-only endpoints: `POST /api/structures/search` with `{query,organism}`; `POST /api/structures/existing` with `{sequence}` or `{accession}`. Sources: [UniProt REST](https://www.uniprot.org/help/api_queries), [AFDB API changes](https://www.ebi.ac.uk/pdbe/news/breaking-changes-afdb-predictions-api), [3D-Beacons](https://www.ebi.ac.uk/pdbe/pdbe-kb/3dbeacons/docs). AFDB data attribution: Google DeepMind / EMBL-EBI, CC BY 4.0; Jumper et al. (2021) and Varadi et al. (2021).
