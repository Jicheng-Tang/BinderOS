...
const structureSearch = await readFile(resolve(root, "worker/structure-search.js"), "utf8");
const molecularViewerLibrary = '/*\n'+await readFile(resolve(root, "node_modules/3dmol/LICENSE"), "utf8")+'\n*/\n'+(await readFile(resolve(root, "node_modules/3dmol/build/3Dmol-min.js"), "utf8")).replace(/\/\/# sourceMappingURL=.*$/m,'');
const structureWorkbenchScript = await readFile(resolve(root, "public/structure-workbench.js"), "utf8");
...
await writeFile(resolve(dist, "server/index.js"), `const page = ${JSON.stringify(html)};\nconst examplePdb = ${JSON.stringify(examplePdb)};\nconst molecularViewerLibrary = ${JSON.stringify(molecularViewerLibrary)};\nconst structureWorkbenchScript = ${JSON.stringify(structureWorkbenchScript)};\n${atlas}\n${structureSearch}\n${worker}`);
