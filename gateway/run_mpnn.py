...
if settings.get("fixed_positions"):
    # Upstream indexes positions by sequence order, including for non-1 PDB numbering.
    chains = set(settings["design_chains"] + settings["fixed_chains"])
    fixed_file = job_dir / "fixed_positions.jsonl"
    fixed_file.write_text(json.dumps({"input": {c: settings["fixed_positions"].get(c, []) for c in chains}}) + "\n")
    sys.argv.extend(["--fixed_positions_jsonl", str(fixed_file)])
